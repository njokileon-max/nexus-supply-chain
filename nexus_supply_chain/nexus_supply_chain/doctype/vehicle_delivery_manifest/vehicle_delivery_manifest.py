    ...
    def on_submit(self):
        try:
            ...
            if self.load_plan:
                ...
                self.db_set("approximate_total_distance_km", load_plan_doc.approximate_total_distance_km)
                self.db_set("approximate_fuel_consumption_ltrs", load_plan_doc.approximate_fuel_consumption_ltrs)
                self.db_set("approximate_fuel_cost", load_plan_doc.approximate_fuel_cost)
                self.db_set("actual_fuel_fuelled", load_plan_doc.actual_fuel_fuelled)
                self.db_set("profitability_status", load_plan_doc.profitability_status)

            elif self.truck_plan:
                truck_plan_doc = frappe.get_doc("Crystal Truck Plan", self.truck_plan)
                company_name = frappe.defaults.get_user_default("Company")

                self.db_set("profit_loss", truck_plan_doc.narrowed_gross_profit)
                self.db_set("gross_margin", truck_plan_doc.narrowed_gross_margin_percentage)
                self.db_set("approximate_total_distance_km", truck_plan_doc.approximate_total_distance_km)
                self.db_set("approximate_fuel_consumption_ltrs", truck_plan_doc.approximate_fuel_consumption_ltrs)
                self.db_set("approximate_fuel_cost", truck_plan_doc.approximate_fuel_cost)
                self.db_set("actual_fuel_fuelled", truck_plan_doc.actual_fuel_fuelled)
                self.db_set("profitability_status", truck_plan_doc.profitability_status)
